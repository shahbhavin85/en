...
public class QuotationHandler extends Basehandler {
	...
	public static String getPageName() {
		return QUOTATION_HANDLER;
	}

	@Override
	public String handlerRequest(HttpServletRequest request,
			HttpServletResponse response) {
		String pageType = "quotation.jsp";
		// action to taken
	...
	private String sendSMS(HttpServletRequest request) {
		...
		if(!model.getCustomer().getMobile1().equals("") || !model.getCustomer().getMobile2().equals(""))
			msg = sendQuotationDtlsSMS(model);
		else {
	...
	private MessageModel sendQuotationDtlsSMS(QuotationModel order){
		...
		try{
			...
			String Message = URLEncoder.encode("Thank you for choosing HESH. Your Quotation has been generated, Quotation No: "+order.getBranch().getBillPrefix()+"Q"+Utils.padBillNo(order.getQuotationId())
					+" Dt: "+Utils.convertToAppDateDDMMYY(order.getQuotationdate())+" Qty: "+qty+" Amt: Rs. "+Utils.get2Decimal(total)+". Regards HESHST","UTF-8");
//			String inputLine;
			URL url;
			BufferedReader bf;
...
//		request.setAttribute(CUSTOMERS, customers);
	...
	private void getTodayQuotationDetails(HttpServletRequest request) {
		QuotationModel[] model = getQuotationBroker().getTodayQuotationDetails(Utils.convertToSQLDate(DateUtil.getCurrDt()), Utils.convertToSQLDate(DateUtil.getCurrDt()), (String)request.getSession().getAttribute(ACCESS_POINT));	
		request.setAttribute(FORM_DATA, model);
		return;
	}

...
public class SalesmanQuotationHandler extends Basehandler {

	public static String getPageName() {
		return SALESMAN_QUOTATION_HANDLER;
	}

	@Override
	public String handlerRequest(HttpServletRequest request,
			HttpServletResponse response) {
		String pageType = "salesmanQuotation.jsp";
		// action to taken
...
//		request.setAttribute(CUSTOMERS, customers);
	...
	private void getTodayQuotationDetails(HttpServletRequest request) {
		QuotationModel[] model = getQuotationBroker().getSalesmanQuotationDetails((UserModel) request.getSession().getAttribute(USER));	
		request.setAttribute(FORM_DATA, model);
		return;
	}
	...
	private String addQuotation(HttpServletRequest request) {
		...
		if(msg.getMessages().size()>1 && (msg.getMessages().get(1)).getType().equals(SUCCESS)){
			...
			pageType = "salesmanQuotation.jsp";
			initialize(request);
		}
		request.setAttribute(FORM_DATA, model);
		return pageType;
	...
	private QuotationModel populateDataModel(HttpServletRequest request) {
		...
		model.setSalesman((UserModel)request.getSession().getAttribute(Constant.USER));

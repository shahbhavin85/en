...
public class CustomerEmailRptHandler extends Basehandler {

	public static String getPageName() {
		return CUSTOMER_EMAIL_RPT_HANDLER;
	}

	@Override
	public String handlerRequest(HttpServletRequest request,
			HttpServletResponse response) {
		String pageType = "customerEmailRpt.jsp";
		// action to taken
	...
	public String getMasterReport(HttpServletRequest request) {
		...
		CustomerEmailModel[] reportBills = getCustomerEmailRptBroker().getMasterRpt(customer,fromdt,todt);
	...
	public String printMasterReport(HttpServletRequest request) {
		String pageType = "printSalesMasterRpt.jsp";
		String customer = (request.getParameter("sCustomer").equals("--")) ? "" : request.getParameter("sCustomer");
		request.setAttribute("sCustomer", request.getParameter("sCustomer"));
		if(!customer.equals("")){
		...
		StringTokenizer payModeST = new StringTokenizer(request.getParameter("sPayMode"),"|");
		ArrayList<String> payModeArr = new ArrayList<String>(0);
		while(payModeST.hasMoreTokens()){
			payModeArr.add(payModeST.nextToken());
		}
		request.setAttribute("sPayMode", (String[])payModeArr.toArray(new String[0]));
		SalesModel[] reportBills = getSalesReportBroker().getMasterRpt(customer,salesman,(String[]) branchArr.toArray(new String[0]),fromdt,todt,
								(String[])billtypeArr.toArray(new String[0]),(String[])payModeArr.toArray(new String[0]));
		request.setAttribute(FORM_DATA, reportBills);
		initialize(request);
		return pageType;
	}
	...
	public String exportMasterReport(HttpServletRequest request) {
		String pageType = "exportSalesMasterRpt.jsp";
		String customer = (request.getParameter("sCustomer").equals("--")) ? "" : request.getParameter("sCustomer");
		request.setAttribute("sCustomer", request.getParameter("sCustomer"));
		if(!customer.equals("")){
			CustomerModel custModel = getCustomerBroker().getCustomerDtls(Integer.parseInt(request.getParameter("sCustomer")));
		...
		String[] payMode = request.getParameterValues("sPayMode"); 
		request.setAttribute("sPayMode", request.getParameterValues("sPayMode"));
		SalesModel[] reportBills = getSalesReportBroker().getMasterRpt(customer,salesman,branch,fromdt,todt,billtype,payMode);
		request.setAttribute(FORM_DATA, reportBills);
		initialize(request);
		return pageType;
	}

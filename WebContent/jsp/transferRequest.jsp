...
<form method="post" name="frmAddCustomer">
	<div class="formClass">
		<fieldset>
			<legend class="screenHeader">Transfer Request</legend>
			...
			<table cellpadding="5" cellspacing="5">
					...
					<td>
						...
						<%
							...
							for(int i=0; i<accessPoints.length; i++){
								...
								if(Integer.parseInt((String)request.getSession().getAttribute(Constant.ACCESS_POINT)) != temp.getAccessId()){
						%>
						<option style="background-color: <%=(k%2==0) ? "#E0E0E0" : "white"%>" value="<%=temp.getAccessId()%>"><%=temp.getAccessName()%></option>
						<%
								k++;
								}
							}
				...
				<tr>
					...
					<td colspan=3><textarea  name="txtRemarks" style="width: 195px; height : 50px;" maxlength="120"></textarea></td>
				</tr>
			</table>
			...
			<table border="1" cellspacing="0" id="enqItem" style="margin-left : 20px; width: 950px;">
				...
				<%
					TransferModel model = (request.getAttribute(Constant.FORM_DATA) != null) ? (TransferModel) request.getAttribute(Constant.FORM_DATA) : new TransferModel();
					TransferItemModel[] transferItems = (TransferItemModel[])model.getItems().toArray(new TransferItemModel[0]);
					for(int i=0; i<transferItems.length; i++){
				%>
				<tr id="s<%=i%>">
					<td align="center"><%=transferItems[i].getItem().getItemId()%></td>
					<td align="center"><%=transferItems[i].getItem().getItemNumber()%></td>
					<td align="center">--</td>
					<td align="center"><%=transferItems[i].getQty()%></td>

	...
	<div class="formClass">
		<fieldset>
			...
			<table style="width: 950px;" align="center">
				<tr>
					<td align="center"><input type="button" value="Edit Request" onclick="editSalesBill('<%=((TransferModel)request.getAttribute(Constant.FORM_DATA)).getFromBranch().getBillPrefix()+"ST"+((TransferModel)request.getAttribute(Constant.FORM_DATA)).getTransferid()%>')" <%=(((TransferModel) request.getAttribute(Constant.FORM_DATA)).isToApproved() || ((TransferModel) request.getAttribute(Constant.FORM_DATA)).isLock()) ? "disabled = \"disabled\"" : "" %> /></td> 
				</tr>
			</table>
		</fieldset>

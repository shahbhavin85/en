		...
		<fieldset>
			...
			<table cellpadding="3" border=1 width="90%" align="center" cellspacing="0">
				...
				<tr>
					...
					<td colspan="3"><b><%=((PurchaseReturnModel)request.getAttribute(Constant.FORM_DATA)).getSupplier().getLabel()%></b></td>
				...
				<tr>
					<td align="right">Remarks</td><td colspan="5"><b><%=((PurchaseReturnModel)request.getAttribute(Constant.FORM_DATA)).getRemark() %></b></td>
				</tr>
			</table>
			...
			<table width="100%">
				<tr>
					<td align="center"><input type="button" value="Close" onclick="javascript: window.close();"/>
					&nbsp;<input type="button" value="Print All" onclick="printSalesBill('<%=((PurchaseReturnModel)request.getAttribute(Constant.FORM_DATA)).getBranch().getBillPrefix()+"PR"+((PurchaseReturnModel)request.getAttribute(Constant.FORM_DATA)).getPurchaseId()%>',0)" />
					...
					&nbsp;<input type="button" value="Email" onclick="emailSales('<%=((PurchaseReturnModel)request.getAttribute(Constant.FORM_DATA)).getBranch().getBillPrefix()+"PR"+((PurchaseReturnModel)request.getAttribute(Constant.FORM_DATA)).getPurchaseId()%>')" /></td>		 
				</tr>
			</table>
		</fieldset>

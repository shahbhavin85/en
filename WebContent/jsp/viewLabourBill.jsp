...
function fnGetSalesDtls(){
	...
	document.forms[0].txtInvoiceNo.value = '<%=(((AccessPointModel)request.getSession().getAttribute(Constant.ACCESS_POINT_DTLS)).getBillPrefix())%>LB'+document.forms[0].txtInvNo.value;
	document.forms[0].HANDLERS.value = "LABOUR_BILL_HANDLER";
	document.forms[0].ACTIONS.value = "GET_LABOUR_BILL";
	fnFinalSubmit();
}
...
<form method="post" name="frmViewEnquiry">
	<div class="formClass">
		<fieldset>
			<legend class="screenHeader">View / Edit Labour Invoice Details</legend>
			<jsp:include page="messages.jsp"></jsp:include>
			<table cellpadding="3">
				<tr>
					<td align="right">Invoice No<span style="color: red">*</span> :</td>
					<td><%=(((AccessPointModel)request.getSession().getAttribute(Constant.ACCESS_POINT_DTLS)).getBillPrefix())+"LB"%> - <input name="txtInvNo" style="width: 195px;" maxlength="5">
			...
			<table width="100%" border="1">
				...
				<%
					...
					for(int i=0; i<todayBills.length;i++){
						gTotal = gTotal + Utils.get2DecimalDouble(todayBills[i].getTotalAmt());
				%>
					<tr>
						<td align="center"><a href="JavaScript:fnGetLinkSalesDtls('<%=((AccessPointModel)request.getSession().getAttribute(Constant.ACCESS_POINT_DTLS)).getBillPrefix()+"LB"+Utils.padBillNo(todayBills[i].getSaleid()) %>')"><%=((AccessPointModel)request.getSession().getAttribute(Constant.ACCESS_POINT_DTLS)).getBillPrefix()+"LB"+Utils.padBillNo(todayBills[i].getSaleid()) %></a></td>
						<td><%=todayBills[i].getCustomer().getCustomerName()+" - "+todayBills[i].getCustomer().getArea()+" - "+todayBills[i].getCustomer().getCity() %></td>
						<td align="right">Rs.<%=Utils.get2Decimal(todayBills[i].getTotalAmt()) %></td>
					</tr>
				<%
					}
				%>

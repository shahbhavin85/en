...
<form method="post" name="frmViewEnquiry">
	<div class="formClass">
		<fieldset>
			<legend class="screenHeader">Purchase Bill Payment</legend>
				...
				<tr>
					...
					<td colspan="3">
						<input type="text" name="txtCust" autocomplete="off" value="<%=(request.getAttribute(Constant.CUSTOMER) != null) ?  
							((CustomerModel)request.getAttribute(Constant.CUSTOMER)).getCustomerName()+" - "+ ((CustomerModel)request.getAttribute(Constant.CUSTOMER)).getArea()+" - "+((CustomerModel)request.getAttribute(Constant.CUSTOMER)).getCity() : "" %>" onkeyup="fnGetPurchaseCustomerList(this);" style="width: 500px;"/><img id="img" src="images/cross.gif" onclick="fnClearCustomer();" style="visibility: <%=(request.getAttribute(Constant.CUSTOMER) != null) ? "visible" : "hidden"%>;"/>
...
<!-- 						</select> -->
				...
				<%
					...
					PurchaseModel[] pendingBills = (request.getAttribute(Constant.PENDING_BILLS_LIST)!=null) ? (PurchaseModel[])request.getAttribute(Constant.PENDING_BILLS_LIST) : new PurchaseModel[0];
				...
				<tr>
					...
					<td colspan="1">
				<%
					if(((String)request.getAttribute("sType")).equals("71")){
				%>
						<input type="radio" onchange="fnPayChange(this);" value="71" name="sType" id="rPaymode1" checked="checked"/><label for="rPaymode1">Cash</label>
				<%
					} else if(((String)request.getAttribute("sType")).equals("73")){
				%>
						<input type="radio" onchange="fnPayChange(this);" value="73" checked="checked" name="sType" id="rPaymode4" /><label for="rPaymode4">Customer Direct Bank Deposit (Cash / Fund Transfer)</label>
				<%
					} else {
				%>
						<input type="radio" onchange="fnPayChange(this);" value="72" checked="checked" name="sType" id="rPaymode3" /><label for="rPaymode3">Cheque</label>
				<%
					}
				%>
					</td>
				<%
					if(((String)request.getAttribute("sType")).equals("72") || ((String)request.getAttribute("sType")).equals("73")){
				%>
				...
				<%
					if(((String)request.getAttribute("sType")).equals("73")){
				%>
				<tr>
				...
				<td align="right"> Transaction No<span style="color: red">*</span> :</td>
				<td colspan="1"><input type="text"  maxlength="10" name="txtChqNo" size="10"/></td>
				</tr>
				<%
					}
					if(((String)request.getAttribute("sType")).equals("72")){
				%>
				<tr>
						...
						<table width="100%" border="1" id="billTable">
							...
							<%
								for(int i=0; i<pendingBills.length; i++){
									k = i;
							%>
							<tr id="<%=pendingBills[i].getBranch().getBillPrefix()+"|"+pendingBills[i].getBranch().getAccessId()+"|"+Utils.padBillNo(pendingBills[i].getPurchaseId()) %>" style="background-color: <%=(i%2==0) ? "#E0E0E0" : "white" %>; color: blue;">
								<td align="center"><input id="v<%=i%>" type="checkbox" onchange="fnPurchaseChkChange(this, 'p<%=i%>','pp<%=i%>','pa<%=i%>')"></td>
								<td align="center"><a style="color: #800517;" href="javascript: fnShowPurchase('<%=pendingBills[i].getBranch().getBillPrefix()+"P"+Utils.padBillNo(pendingBills[i].getPurchaseId()) %>');"><%=pendingBills[i].getBranch().getBillPrefix()+"P"+Utils.padBillNo(pendingBills[i].getPurchaseId()) %></a></td>
								<td align="center"><%=Utils.convertToAppDateDDMMYY(pendingBills[i].getRecdDt()) %></td>
								<td align="center"><%=pendingBills[i].getInvNo() %></td>
								<td align="center"><%=Utils.convertToAppDateDDMMYY(pendingBills[i].getInvDt()) %></td>
								<td align="right"><%=Utils.get2Decimal(Utils.get2DecimalDouble(pendingBills[i].getTotalAmt()+pendingBills[i].getExtra()-pendingBills[i].getDiscount())) %></td>
								...
								<td align="center"><input type="text" size="25" style="text-align: right;" readonly="readonly" id="pp<%=i%>" value=<%=Utils.get2Decimal(Utils.get2DecimalDouble(pendingBills[i].getTotalAmt()+pendingBills[i].getExtra()-pendingBills[i].getDiscount()) - Utils.get2DecimalDouble(pendingBills[i].getPayAmt()) ) %> /></td>
								<td align="center"><input type="text" size="25" style="text-align: right;" onblur="fnAmtChange(this, 'pp<%=i%>', 'v<%=i%>');" id="p<%=i%>" readonly="readonly" onkeypress="return numbersonly(this, event, true)" name="p<%=pendingBills[i].getBranch().getBillPrefix()+"P"+Utils.padBillNo(pendingBills[i].getPurchaseId())%>" id="p<%=pendingBills[i].getBranch().getBillPrefix()+"P"+Utils.padBillNo(pendingBills[i].getPurchaseId())%>" value="0"/></td>
								<td align="center"><input type="text" size="25" style="text-align: right;" onblur="fnAdjAmtChange(this, 'p<%=i%>', 'pp<%=i%>', 'v<%=i%>');" id="pa<%=i%>" readonly="readonly" name="pa<%=pendingBills[i].getBranch().getBillPrefix()+"P"+Utils.padBillNo(pendingBills[i].getPurchaseId())%>" id="p<%=pendingBills[i].getBranch().getBillPrefix()+"P"+Utils.padBillNo(pendingBills[i].getPurchaseId())%>" value="0"/></td>
							</tr>
							<%
								}
							%>
				...
				<tr>
					<td colspan="4" align="center"><input type="button" name="btnSave" disabled="disabled" value="Save" onclick="fnSavePurchasePayment();"/> <input type="button" name="btnBack" value="Back" onclick="fnBack();"/> </td>
				</tr>
				<%
					}
				%>

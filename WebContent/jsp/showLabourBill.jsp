...
function editSalesBill(billNo){
	...
	document.forms[0].ACTIONS.value = "INIT_EDIT_LABOUR_BILL";
	fnFinalSubmit();
}
</script>
	...
	<div class="formClass">
		<fieldset>
			...
			<table style="width: 950px;" align="center">
				<tr>
					<td align="center"><input type="button" value="Print All" onclick="printSalesBill('<%=((LabourBillModel)request.getAttribute(Constant.FORM_DATA)).getBranch().getBillPrefix()+"LB"+((LabourBillModel)request.getAttribute(Constant.FORM_DATA)).getSaleid()%>',0)" />
					...
					&nbsp;<input type="button" value="Edit Bill" onclick="editSalesBill('<%=((LabourBillModel)request.getAttribute(Constant.FORM_DATA)).getBranch().getBillPrefix()+"LB"+((LabourBillModel)request.getAttribute(Constant.FORM_DATA)).getSaleid()%>')" />

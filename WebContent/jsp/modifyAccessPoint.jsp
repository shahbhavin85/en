...
<form method="post" name="frmAddAccessPoint">
	<div class="formClass">
		<fieldset>
			<legend class="screenHeader">Modify Access Point</legend>
			<jsp:include page="messages.jsp"></jsp:include>
			<table cellpadding="3">
				<tr>
					<td align="right">Select Access Point :</td>
					<td>
						<select name="sAccessName" style="width: 200px;" onchange="fnGetAccessPointDetails();">
							<%
							...
							<option value="<%=temp.getAccessId()%>" <%=(request.getAttribute(Constant.FORM_DATA) != null && 
								((AccessPointModel)request.getAttribute(Constant.FORM_DATA)).getAccessId() == temp.getAccessId()) ? "selected=\"selected\"" : "" %>><%=temp.getAccessName()%></option>
							<%
								}
							%>
						</select>
				...
				<tr>
					<td colspan="2" align="center"><input type="button" onclick="fnUpdateAccessPointDetails();" <%=((request.getAttribute(Constant.FORM_DATA) != null && !request.getSession().getAttribute(Constant.ACCESS_POINT).equals("0")) || request.getAttribute(Constant.FORM_DATA) == null) ? "disabled=\"disabled\"" : ""%>
						value="Modify"></td>
				</tr>
			</table>
		</fieldset>

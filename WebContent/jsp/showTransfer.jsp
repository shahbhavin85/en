	...
	<div class="formClass">
		<fieldset>
			...
			<table style="width: 950px;" align="center">
				<tr>
					<td align="center"><input type="button" value="Print All" onclick="printSalesBill('<%=((TransferModel)request.getAttribute(Constant.FORM_DATA)).getFromBranch().getBillPrefix()+"ST"+((TransferModel)request.getAttribute(Constant.FORM_DATA)).getTransferid()%>',0)" />
					&nbsp;<input type="button" value="Original Copy" onclick="printSalesBill('<%=((TransferModel)request.getAttribute(Constant.FORM_DATA)).getFromBranch().getBillPrefix()+"ST"+((TransferModel)request.getAttribute(Constant.FORM_DATA)).getTransferid()%>',1)" />
					&nbsp;<input type="button" value="Transport Copy" onclick="printSalesBill('<%=((TransferModel)request.getAttribute(Constant.FORM_DATA)).getFromBranch().getBillPrefix()+"ST"+((TransferModel)request.getAttribute(Constant.FORM_DATA)).getTransferid()%>',2)" />
					&nbsp;<input type="button" value="Office Copy" onclick="printSalesBill('<%=((TransferModel)request.getAttribute(Constant.FORM_DATA)).getFromBranch().getBillPrefix()+"ST"+((TransferModel)request.getAttribute(Constant.FORM_DATA)).getTransferid()%>',3)" />
					&nbsp;<input type="button" value="Edit Bill" onclick="editSalesBill('<%=((TransferModel)request.getAttribute(Constant.FORM_DATA)).getFromBranch().getBillPrefix()+"ST"+((TransferModel)request.getAttribute(Constant.FORM_DATA)).getTransferid()%>')" <%=(((TransferModel) request.getAttribute(Constant.FORM_DATA)).isToApproved() || ((TransferModel) request.getAttribute(Constant.FORM_DATA)).isLock()) ? "disabled = \"disabled\"" : "" %> /></td> 
				</tr>
			</table>
		</fieldset>

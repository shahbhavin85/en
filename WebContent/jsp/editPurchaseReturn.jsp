...
<form method="post" name="frmAddCustomer">
	<div class="formClass">
		<fieldset>
			<legend class="screenHeader">Purchase Return Invoice</legend>
			<jsp:include page="messages.jsp"></jsp:include>
			<table cellpadding="0">
				<tr>
					<td align="right">Purchase Return Id<span style="color: red">*</span> :</td>
					<td colspan="5"><input name="txtInvoiceNo" readonly="readonly" style="width: 195px;" value="<%=((PurchaseReturnModel)request.getAttribute(Constant.FORM_DATA)).getBranch().getBillPrefix()+"PR"+Utils.padBillNo(((PurchaseReturnModel)request.getAttribute(Constant.FORM_DATA)).getPurchaseId())%>"></td>
				</tr>
				<tr>
					...
					<td colspan="3">
						<select name="sCustomer">
							...
							<option value="<%=customers[i].getCustomerId()%>" style="background-color: <%=(i%2==0) ? "#E0E0E0" : "white"%>"  <%=(request.getAttribute(Constant.FORM_DATA) != null && 
									((PurchaseReturnModel)request.getAttribute(Constant.FORM_DATA)).getSupplier().getCustomerId() == customers[i].getCustomerId()) ? "selected=\"selected\"" : "" %>><%=customers[i].getCustomerName()%> - <%=customers[i].getArea()%> - <%=customers[i].getCity()%></option>
							<%
								}
							%>
						</select>
                 ...
                 <tr>
					...
					<td colspan="3">
						<input type="radio" value="1" name="rTaxType" id="rTaxType1" <%=(request.getAttribute(Constant.FORM_DATA) != null && 
									((PurchaseReturnModel)request.getAttribute(Constant.FORM_DATA)).getBillType() == 1) ? "checked=\"checked\"" : "" %> <%=(request.getAttribute(Constant.FORM_DATA) == null) ? "checked=\"checked\"" : ""%> /><label for="rTaxType1" style="padding: 2px; color: red; background-color: yellow;">VAT</label>
						<input type="radio" value="2" name="rTaxType" id="rTaxType2" <%=(request.getAttribute(Constant.FORM_DATA) != null && 
									((PurchaseReturnModel)request.getAttribute(Constant.FORM_DATA)).getBillType() == 2) ? "checked=\"checked\"" : "" %>/><label for="rTaxType2" style="padding: 2px; color: white; background-color: black;">CST</label>
						<input type="radio" value="3" name="rTaxType" id="rTaxType3" <%=(request.getAttribute(Constant.FORM_DATA) != null && 
									((PurchaseReturnModel)request.getAttribute(Constant.FORM_DATA)).getBillType() == 3) ? "checked=\"checked\"" : "" %>/><label for="rTaxType3" style="padding: 2px; color: orange; background-color: brown;">CST Against from 'C'</label>
					</td>
				</tr>
			</table>
			...
			<table border="1" cellspacing="0" id="enqItem" style="margin-left : 20px; width: 950px;">
				...
				<tr id=s<%=i%>>
					...
					<td align="center"><%=Utils.get2Decimal(salesItems[i].getQty() * salesItems[i].getRate() * ((100+salesItems[i].getTax())/100))%></td>
				</tr>
				<%
					}
				%>
			...
			<table style="margin-top : 10px; margin-left : 20px; width: 950px;" cellspacing="3" cellpadding="0">
				<tr>
					...
					<td valign="top" style="padding-top: 3px;"><textarea  name="txtRemarks" style="width: 550px; height : 50px;" maxlength="120"><%=((request.getAttribute(Constant.FORM_DATA)) != null) ? ((PurchaseReturnModel)request.getAttribute(Constant.FORM_DATA)).getRemark() : ""%></textarea></td>
					<td align="right" valign="top">Grand Total (With Taxes) :</td>
			...
			<table width="80%">
				<tr>
					<td align="center"><input type="button" name="genBill" value="Edit & Generate Purchase Memo" onclick="fnEditGenerateBill();" /></td> 
				</tr>
			</table>
		</fieldset>

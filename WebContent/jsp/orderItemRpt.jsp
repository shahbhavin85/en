...
<form method="post" name="frmViewEnquiry">
	<div class="formClass">
		<fieldset>
			<legend class="screenHeader">Order Item Report</legend>
...
<!-- 					</td> -->
				...
				<tr>
					<td colspan="2" align="center"><input type="button" value="Get Details" onclick="fnItemMasterRpt();"/></td>
				</tr>
			</table>
			<%
				...
				HashMap<String, OrderModel[]> todayBills = (request.getAttribute(Constant.FORM_DATA)!=null) ? (HashMap<String, OrderModel[]>) request.getAttribute(Constant.FORM_DATA) : new HashMap<String, OrderModel[]>(0);
...
<!-- 				<table width="100%"><tr><td align="right"><img alt="export to xls" src="images/xls.gif;" style="padding: 2px;" onmouseover="this.style.background='gray';" onmouseout="this.style.background='transparent';"/><span style="padding: 2px; vertical-align: " onmouseover="this.style.background='gray';" onmouseout="this.style.background='transparent';"><img alt="export to xls" src="images/pdf.gif;"/>PDF</span></td></tr></table> -->
			<table width="100%" border="1" cellspacing="2" style="font-size: 11px;">
				<tr>
					<th colspan="12" align="center">Order Item Details</th>
				</tr>
				<tr>
				...
				%>
					<tr style="background-color: black; color: white;">
						<th><%=item.getItem().getItemId()%></th>
						<th colspan="2"><%=item.getItem().getItemName() + " - " +item.getItem().getItemNumber()%></th>
						<th><%=item.getQty()%></th>
					</tr>
				<%
						model = todayBills.get(key);
						for(int i=0; i<model.length; i++){
				%>
					<tr style="background-color: <%=(i%2==0) ? "#E0E0E0" : "white" %>; color: blue;">
						<td align="center" valign="top"><a style="color: #800517;" href="javascript: fnShowSales('<%=model[i].getBranch().getBillPrefix()+"O"+Utils.padBillNo(model[i].getOrderId()) %>');"><%=model[i].getBranch().getBillPrefix()+"O"+Utils.padBillNo(model[i].getOrderId()) %></a></td>
						<td align="center" valign="top"><%=Utils.convertToAppDateDDMMYY(model[i].getOrderDate()) %></td>
						<td><%=model[i].getCustomer().getCustomerName()+" - "+model[i].getCustomer().getArea()+" - "+model[i].getCustomer().getCity() %></td>
						<td align="center" valign="top"><%=model[i].getItems().get(0).getQty()%></td>
					</tr>
				<%
						}
					}

...
<script type="text/javascript" src="js/purchaseRpt.js"></script>
</head>
<body>
		...
		<fieldset>
			...
			<table width="100%" border="1" cellspacing="2" style="font-size: 11px;">
				...
				<tr>
					...
					<th style="width: 50px;">RECD DATE</th>
					<th style="width: 80px;">INVOICE NO</th>
					<th style="width: 50px;">INVOICE DT</th>
				...
				<%
					...
					for(int i=0; i<todayBills.length;i++){
						gTotal = gTotal + Utils.get2DecimalDouble(Utils.get2DecimalDouble(todayBills[i].getTotalAmt()+todayBills[i].getExtra()-todayBills[i].getDiscount()));
						fontColor="blue";
						pendintTotal = pendintTotal + (Utils.get2DecimalDouble(Utils.get2DecimalDouble(todayBills[i].getTotalAmt()+todayBills[i].getExtra()-todayBills[i].getDiscount())-Utils.get2DecimalDouble(todayBills[i].getPayAmt())));
						paidTotal = paidTotal + (Utils.get2DecimalDouble(Utils.get2DecimalDouble(todayBills[i].getPayAmt())));
						taxBackColor = (todayBills[i].getBillType() == 1) ? "#FFC1C1" : (todayBills[i].getBillType() == 2) ? "#FFFF7E" : "#BCED91";
				%>
					<tr style="background-color: <%=(i%2==0) ? "#E0E0E0" : "white" %>; color: <%=fontColor%>;">
						<td align="center" valign="top"><a style="color: #800517;" href="javascript: fnShowPurchase('<%=todayBills[i].getBranch().getBillPrefix()+"P"+Utils.padBillNo(todayBills[i].getPurchaseId()) %>');"><%=todayBills[i].getBranch().getBillPrefix()+"P"+Utils.padBillNo(todayBills[i].getPurchaseId()) %></a></td>
						<td align="center" valign="top"><%=Utils.convertToAppDateDDMMYY(todayBills[i].getRecdDt()) %></td>
						<td align="center" valign="top"><%=todayBills[i].getInvNo()%></td>
						<td align="center" valign="top"><%=Utils.convertToAppDateDDMMYY(todayBills[i].getInvDt()) %></td>
						...
						<td align="right" valign="top"><%=Utils.get2Decimal(Utils.get2DecimalDouble(todayBills[i].getTotalAmt()+todayBills[i].getExtra()-todayBills[i].getDiscount())) %></td>
						...
						<td align="right" valign="top"><%=Utils.get2Decimal(Utils.get2DecimalDouble(todayBills[i].getPayAmt())) %></td>
						<td align="right" valign="top"><%=((Double.parseDouble(Utils.get2Decimal(Utils.get2DecimalDouble(todayBills[i].getTotalAmt()+todayBills[i].getExtra()-todayBills[i].getDiscount())-todayBills[i].getPayAmt())) != 0)) ? Utils.get2Decimal(Utils.get2DecimalDouble(todayBills[i].getTotalAmt()+todayBills[i].getExtra()-todayBills[i].getDiscount())-todayBills[i].getPayAmt()) : "--"%></td>
					</tr>
				<%
					}
				%>

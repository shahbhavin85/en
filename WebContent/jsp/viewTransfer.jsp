...
function fnGetTransferDtls(){
	...
	document.forms[0].ACTIONS.value = "GET_TRANSFER";
	fnFinalSubmit();
}
...
<form method="post" name="frmViewEnquiry">
	<div class="formClass">
		<fieldset>
			<legend class="screenHeader">View / Edit Stock Transfer Details</legend>
			<jsp:include page="messages.jsp"></jsp:include>
			<table cellpadding="3">
				<tr>
					<td align="right">Stock Transfer No<span style="color: red">*</span> :</td>
					<td><%=(((AccessPointModel)request.getSession().getAttribute(Constant.ACCESS_POINT_DTLS)).getBillPrefix())+"ST"%> - <input name="txtInvNo" maxlength="5" style="width: 195px;">
			...
			<table width="100%" border="1">
				...
				%>
					<tr>
						<td align="center"><a href="JavaScript:fnGetLinkTransferDtls('<%=((AccessPointModel)request.getSession().getAttribute(Constant.ACCESS_POINT_DTLS)).getBillPrefix()+"ST"+Utils.padBillNo(todayBills[i].getTransferid()) %>')"><%=((AccessPointModel)request.getSession().getAttribute(Constant.ACCESS_POINT_DTLS)).getBillPrefix()+"ST"+Utils.padBillNo(todayBills[i].getTransferid()) %></a></td>

...
<form method="post" name="frmUserHome">
	<div class="formClass">
		<fieldset>
			<legend class="screenHeader">New Inward Entry</legend><br>
			...
			<table style="width: 950px;">
				<tr><td style="padding-left: 15px;"> China Name: <br/>
					<select name="sItemName" style="width: 320px;" onchange="fnChangeItemNo(this);">
						...
						%>
							<option id="<%=items[i].getItemPrice() %>" style="background-color: <%=(i%2==0) ? "#E0E0E0" : "white"%>" value="<%=items[i].getItemId()%>"><%=items[i].getRefName()%></option>
						<%
							}
						%>
					</select></td>
			...
			<table border="1" cellspacing="0" id="enqItem" style="margin-left : 20px; width: 950px;">
				...
				<tr id=s<%=i%>>
					...
					<td align="center"><%=Utils.get2Decimal((model.getExchangeRate()*model.getSourceExp()/model.getCbm())*salesItems[i].getCbm())%></td>
					<td align="center"><%=Utils.get2Decimal((model.getDestinationExp()/model.getCbm())*salesItems[i].getCbm())%></td>
					<td align="center"><%=Utils.get2Decimal(salesItems[i].getRate()+((((model.getDestinationExp()+(model.getExchangeRate()*model.getSourceExp()))/model.getCbm()))*salesItems[i].getCbm()/salesItems[i].getQty()))%></td>
				</tr>
				<%
					}
				%>
			...
			<table width="80%">
				<tr>
					<td align="center"><input type="button" name="genBill" value="Save Entry" onclick="fnGenerateBill();" /></td> 
				</tr>
			</table>
		</fieldset>

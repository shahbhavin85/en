...
<form method="post" name="frmAddCustomer">
	<div class="formClass">
		<fieldset>
			<legend class="screenHeader">Edit Order Form</legend>
			<jsp:include page="messages.jsp"></jsp:include>
			<table cellpadding="3">
				<tr>
					<td align="right">Order No<span style="color: red">*</span> :</td>
					<td colspan="5"><input name="txtInvoiceNo" readonly="readonly" style="width: 70px;" value="<%=((OrderModel)request.getAttribute(Constant.FORM_DATA)).getBranch().getBillPrefix()+"O"+Utils.padBillNo(((OrderModel)request.getAttribute(Constant.FORM_DATA)).getOrderId())%>">
						&nbsp;&nbsp;&nbsp;Branch State : <b><label style="background: aqua; color: brown; padding: 2px;" id="branchState"><%=((OrderModel)request.getAttribute(Constant.FORM_DATA)).getBranch().getState() %></label></b>
...
<!-- 							%> -->
<%-- 							<option id='<%=temp.getId() %>' style="background-color: <%=(i%2==0) ? "#E0E0E0" : "white"%>" --%>
<%-- 								 value="<%=temp.getCustomerId()%>"  <%=(request.getAttribute(Constant.FORM_DATA) != null &&  --%>
<%-- 									((OrderModel)request.getAttribute(Constant.FORM_DATA)).getCustomer().getCustomerId() == temp.getCustomerId()) ? "selected=\"selected\"" : "" %>><%=temp.getCustomerName()%> - <%=temp.getArea()%></option> --%>
<%-- 							<% --%>
<!-- 								} -->
<!-- 							%> -->
<!-- 						</select> -->
			...
			<table style="margin-top : 10px; margin-left : 10px; width: 950px;" cellspacing="3" cellpadding="0">
				<tr>
					...
					<td rowspan="3" valign="top" style="padding-top: 3px;"><textarea  name="txtRemarks" style="width: 550px; height : 60px;" maxlength="300"><%=(request.getAttribute(Constant.FORM_DATA) != null) ?  
							((OrderModel)request.getAttribute(Constant.FORM_DATA)).getRemarks() : "" %></textarea></td>
					<td align="right">Installation Charges (+) :</td>
					<td  style="width: 120px;"><input type="text" style="width: 110px; text-align: right;" name="txtInstall" value = "<%=(request.getAttribute(Constant.FORM_DATA) != null) ? ((OrderModel)request.getAttribute(Constant.FORM_DATA)).getInstallation() : 0%>" onblur="fnChangeTotal(this);" onkeypress="return numbersonly(this, event, true);s"></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td rowspan="2" valign="top" style="padding-top: 3px;"><textarea  name="txtDevAdd" style="width: 550px; height : 60px;" maxlength="300"><%=(request.getAttribute(Constant.FORM_DATA) != null) ?  
							((OrderModel)request.getAttribute(Constant.FORM_DATA)).getDevAddress() : "" %></textarea></td>
	...
	<input type="hidden" name="sCustomer" value="<%=((OrderModel)request.getAttribute(Constant.FORM_DATA)).getCustomer().getCustomerId()%>">

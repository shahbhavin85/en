...
response.setHeader("Content-disposition","inline; filename=sales_master_"+DateUtil.getCurrDt()+".xls");
...
<%
try {
SalesModel[] dtls = (SalesModel[]) request.getAttribute("FORM_DATA");
...
<td><%=model.getTaxtype() %></td>
<td><%=model.getPaymode() %></td>
<td><%=model.getPacking() %></td>
<td><%=model.getForwarding() %></td>
<td><%=model.getInstallation() %></td>
<td><%=model.getCess() %></td>
<td><%=model.getLess() %></td>

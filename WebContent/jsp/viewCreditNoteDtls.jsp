...
function printSalesBill(billNo, copyType){
	window.open('/en/app?HANDLERS=CREDIT_NOTE_HANDLER&ACTIONS=PRNT_CREDIT_NOTE&billNo='+billNo+'&copies='+copyType,"","status=1,scrollbars=1, top=0, left=0,width="+(screen.width-10)+",height="+(screen.height-120));
}
		...
		<fieldset>
			...
			<table width="100%">
				<tr>
					...
					<input type="button" value="Print All" onclick="printSalesBill('<%=((CreditNoteModel)request.getAttribute(Constant.FORM_DATA)).getBranch().getBillPrefix()+"CN"+((CreditNoteModel)request.getAttribute(Constant.FORM_DATA)).getSaleid()%>',0)" />

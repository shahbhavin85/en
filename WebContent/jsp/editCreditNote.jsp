...
<form method="post" name="frmAddCustomer">
	<div class="formClass">
		<fieldset>
			<legend class="screenHeader">Edit Credit Note Invoice</legend>
			<jsp:include page="messages.jsp"></jsp:include>
			<table cellpadding="3">
				<tr>
					<td align="right">Note No<span style="color: red">*</span> :</td>
					<td colspan="3"><input name="txtInvoiceNo" readonly="readonly" style="width: 75px;" value="<%=((CreditNoteModel)request.getAttribute(Constant.FORM_DATA)).getBranch().getBillPrefix()+"CN"+Utils.padBillNo(((CreditNoteModel)request.getAttribute(Constant.FORM_DATA)).getSaleid())%>">
						&nbsp;&nbsp;&nbsp;Branch State : <b><label style="background: aqua; color: brown; padding: 2px;" id="branchState"><%=((CreditNoteModel)request.getAttribute(Constant.FORM_DATA)).getBranch().getState() %></label></b>
...
<%-- 									((CreditNoteModel)request.getAttribute(Constant.FORM_DATA)).getCustomer().getCustomerId() == customers[i].getCustomerId()) ? "selected=\"selected\"" : "" %>><%=customers[i].getCustomerName()%> - <%=customers[i].getArea()%> - <%=customers[i].getCity()%></option> --%>
<%-- 							<% --%>
<!-- 								} -->
<!-- 							%> -->
<!-- 						</select> -->
			...
			<table style="margin-top : 10px; margin-left : 10px; width: 950px;" cellspacing="3" cellpadding="0">
				<tr>
					...
					<td style="width: 120px;"><input type="text" style="width: 110px; text-align: right;" onblur="fnChangeTotal(this);" name="txtLess" value = "<%=(request.getAttribute(Constant.FORM_DATA) != null) ? ((CreditNoteModel)request.getAttribute(Constant.FORM_DATA)).getLess() : 0%>" onkeypress="return numbersonly(this, event, true)"></td>
				</tr>
				<tr>
	...
	<input type="hidden" name="sCustomer" value="<%=((CreditNoteModel)request.getAttribute(Constant.FORM_DATA)).getCustomer().getCustomerId()%>">

		...
		<fieldset>
			...
			<table cellpadding="2" style="width: 950px;">
				<tr>
					...
					<td colspan="3">
						<b><label style="background: aqua; color: brown; padding: 2px;" id="branchState"><%=((AccessPointModel)request.getSession().getAttribute(Constant.ACCESS_POINT_DTLS)).getState() %></label></b>
...
<%-- 									((QuotationModel)request.getAttribute(Constant.FORM_DATA)).getCustomer().getCustomerId() == temp.getCustomerId()) ? "selected=\"selected\"" : "" %>><%=temp.getCustomerName()%> - <%=temp.getArea()%></option> --%>
<%-- 							<% --%>
<!-- 								} -->
<!-- 							%> -->
<!-- 						</select> -->

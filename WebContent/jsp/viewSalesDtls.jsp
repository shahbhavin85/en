...
<form method="post" name="frmAddCustomer">
	...
	<div class="formClass">
		<fieldset>
			...
			<table width="100%">
				<tr>
					<td align="center"><input type="button" value="Close" onclick="javascript: window.close();"/><input type="button" value="Print" onclick="printSalesBill('<%=((SalesModel)request.getAttribute(Constant.FORM_DATA)).getBranch().getBillPrefix()+((SalesModel)request.getAttribute(Constant.FORM_DATA)).getSaleid()%>',1)"/><input type="button" value="Email" onclick="fnEmail();"/><input type="button" value="SMS" onclick="fnSMS();"/><input type="button" value="Ledger" onclick="fnLedger('<%=((SalesModel)request.getAttribute(Constant.FORM_DATA)).getCustomer().getCustomerId()%>')"/></td>		 
				</tr>
			</table>
		</fieldset>

...
<form method="post" name="frmViewEnquiry">
	<div class="formClass">
		<fieldset>
			<legend class="screenHeader">Labour Bill Collection</legend>
				...
				<tr>
					...
					<td colspan="3">
						<input type="text" name="txtCust" autocomplete="off" value="<%=(request.getAttribute(Constant.CUSTOMER) != null) ?  
							((CustomerModel)request.getAttribute(Constant.CUSTOMER)).getCustomerName()+" - "+ ((CustomerModel)request.getAttribute(Constant.CUSTOMER)).getArea()+" - "+((CustomerModel)request.getAttribute(Constant.CUSTOMER)).getCity() : "" %>" onkeyup="fnGetBillCustomerList(this);" style="width: 500px;"/><img id="img" src="images/cross.gif" onclick="fnClearCustomer();" style="visibility: <%=(request.getAttribute(Constant.CUSTOMER) != null) ? "visible" : "hidden"%>;"/>
						<div id="list" class="suggestionsBox">
							<div class="suggestionList" id="autoSuggestionsList"></div>
						</div>
<!-- 						<select name="sCustomer" onchange="fnGetPendingBills();"> -->
<!-- 							<option value="--">-------</option> -->
<%-- 							<% --%>
...
<!-- 						</select></td> -->
				...
				<%
					...
					LabourBillModel[] pendingBills = (request.getAttribute(Constant.PENDING_BILLS_LIST)!=null) ? (LabourBillModel[])request.getAttribute(Constant.PENDING_BILLS_LIST) : new LabourBillModel[0];
				...
				<tr>
					...
					<td colspan="1">
				<%
					if(((String)request.getAttribute("sType")).equals("46")){
				%>
						<input type="radio" onchange="fnPayChange(this);" value="46" name="sType" id="rPaymode1" checked="checked"/><label for="rPaymode1">Cash</label>
				<%
					} else if(((String)request.getAttribute("sType")).equals("47")){
				%>
						<input type="radio" onchange="fnPayChange(this);" value="47" checked="checked" name="sType" id="rPaymode2" /><label for="rPaymode2">CC Card</label>
				<%
					} else if(((String)request.getAttribute("sType")).equals("49")){
				%>
						<input type="radio" onchange="fnPayChange(this);" value="49" checked="checked" name="sType" id="rPaymode4" /><label for="rPaymode4">Customer Direct Bank Deposit (Cash / Fund Transfer)</label>
				<%
					} else {
				%>
						<input type="radio" onchange="fnPayChange(this);" value="48" checked="checked" name="sType" id="rPaymode3" /><label for="rPaymode3">Cheque</label>
				<%
					}
				%>
					</td>
				...
				<%
					if(((String)request.getAttribute("sType")).equals("48")){
				%>
				<tr>
				...
				<tr>
					...
					<td colspan="1"><input type="text" onblur="fnRefreshPending();" onkeypress="return numbersonly(this, event, true)" maxlength="13" name="txtAmount" size="25" value="0"/></td>
					...
					<td colspan="4">
						<table width="100%" border="1" id="billTable">
							...
							<%
								...
								for(int i=0; i<pendingBills.length; i++){
									k = i;
									roundoff = (1-Utils.get2DecimalDouble((pendingBills[i].getTotalAmt())%1) < 0.51) ? Utils.get2DecimalDouble(1-Utils.get2DecimalDouble((pendingBills[i].getTotalAmt())%1)) : - Utils.get2DecimalDouble(Utils.get2DecimalDouble((pendingBills[i].getTotalAmt())%1)) ;
							%>
							<tr id="<%=pendingBills[i].getBranch().getBillPrefix()+"|"+pendingBills[i].getBranch().getAccessId()+"|"+Utils.padBillNo(pendingBills[i].getSaleid()) %>" style="background-color: <%=(i%2==0) ? "#E0E0E0" : "white" %>; color: blue;">
								<td align="center"><input id="v<%=i%>" type="checkbox" onchange="fnChkChange(this, 'p<%=i%>','pp<%=i%>','pa<%=i%>')"></td>
								<td align="center"><a style="color: #800517;" href="javascript: fnShowLabourSales('<%=pendingBills[i].getBranch().getBillPrefix()+"LB"+Utils.padBillNo(pendingBills[i].getSaleid()) %>');"><%=pendingBills[i].getBranch().getBillPrefix()+"LB"+Utils.padBillNo(pendingBills[i].getSaleid()) %></a></td>
								<td align="center"><%=Utils.convertToAppDateDDMMYY(pendingBills[i].getSalesdate()) %></td>
								<td align="right"><%=Utils.get2Decimal(Utils.get2DecimalDouble(pendingBills[i].getTotalAmt())+roundoff) %></td>
								...
								<td align="center"><input type="text" size="25" style="text-align: right;" readonly="readonly" id="pp<%=i%>" value=<%=Utils.get2Decimal(Utils.get2DecimalDouble(Utils.get2DecimalDouble(pendingBills[i].getTotalAmt())+roundoff) - Utils.get2DecimalDouble(pendingBills[i].getPayAmt()) ) %> /></td>
								<td align="center"><input type="text" size="25" style="text-align: right;" onblur="fnAmtChange(this, 'pp<%=i%>', 'v<%=i%>');" id="p<%=i%>" readonly="readonly" onkeypress="return numbersonly(this, event, true)" name="p<%=pendingBills[i].getBranch().getBillPrefix()+Utils.padBillNo(pendingBills[i].getSaleid())%>" id="p<%=pendingBills[i].getBranch().getBillPrefix()+Utils.padBillNo(pendingBills[i].getSaleid())%>" value="0"/></td>
								<td align="center"><input type="text" size="25" style="text-align: right;" onblur="fnAdjAmtChange(this, 'p<%=i%>', 'pp<%=i%>', 'v<%=i%>');" id="pa<%=i%>" readonly="readonly" name="pa<%=pendingBills[i].getBranch().getBillPrefix()+Utils.padBillNo(pendingBills[i].getSaleid())%>" id="p<%=pendingBills[i].getBranch().getBillPrefix()+Utils.padBillNo(pendingBills[i].getSaleid())%>" value="0"/></td>
							</tr>
							<%
								}
							%>
				...
				<tr>
					<td colspan="4" align="center"><input type="button" name="btnSave" disabled="disabled" value="Save" onclick="fnSaveLabourBill();"/> <input type="button" name="btnBack" value="Back" onclick="fnBack();"/> </td>
				</tr>
				<%
					}
				%>

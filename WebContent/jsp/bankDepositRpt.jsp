...
function fnGetReport(){
	...
	document.forms[0].ACTIONS.value = 'GET_BANK_ENTRIES_RPT';
	fnFinalSubmit();
}
...
function fnPrint(){
	...
	window.open('/en/app?HANDLERS=BRANCH_DLY_ENTRY_RPT_HANDLER&ACTIONS=PRNT_BANK_ENTRIES_RPT'+params,"","status=1,scrollbars=1, top=0, left=0,width="+(screen.width-10)+",height="+(screen.height-120));
}

function fnExport(){
	document.forms[0].HANDLERS.value = "BRANCH_DLY_ENTRY_RPT_HANDLER";
	document.forms[0].ACTIONS.value = "EXPT_BANK_ENTRIES_RPT";
...
<form method="post" name="frmViewEnquiry">
	<div class="formClass">
		<fieldset>
			<legend class="screenHeader">Bank Direct Deposit Entries Report</legend>
...
<!-- 					<td colspan="3"><select name="sCustomer"> -->
<!-- 							<option value="--">-----</option> -->
<%-- 							<% --%>
<!-- 								String state = ""; -->
				...
				<tr>
					...
					%>
						...
						<tr>
							...
							<td colspan="2"><input type="checkbox" onchange="fnChangeBranch(this);" name="cBranch" <%=((accessSelected.length > 0 && (accessSelected[0].equals("--") || selected.indexOf("2")!=-1) || accessSelected.length == 0)) ? "checked=\"checked\"" : ""%> id="cBranch100" value="2"/><label for="cBranch100">Accounts</label></td>
						</tr>
					<% } %>
						<%
							if(!request.getSession().getAttribute(Constant.ACCESS_POINT).equals("0") && !request.getSession().getAttribute(Constant.ACCESS_POINT).equals("2")){

...
function fnGenerateBill(){
	...
	document.forms[0].ACTIONS.value = "ADD_PURCHASE_RETURN";
	fnFinalSubmit();
}
...
function printSalesBill(billNo, copyType){
	window.open('/en/app?HANDLERS=PURCHASE_RETURN_HANDLER&ACTIONS=PRNT_PURCHASE_RETURN&billNo='+billNo+'&copies='+copyType,"","status=1,scrollbars=1, top=0, left=0,width="+(screen.width-10)+",height="+(screen.height-120));
}

function editPurchaseBill(billNo){
	document.forms[0].txtInvoiceNo.value = billNo;
	document.forms[0].HANDLERS.value = "PURCHASE_RETURN_HANDLER";
	document.forms[0].ACTIONS.value = "INIT_EDIT_PURCHASE_RETURN";
	fnFinalSubmit();
}
...
function fnEditGenerateBill(){
	...
	buildSalesItemData();
	document.forms[0].HANDLERS.value = "PURCHASE_RETURN_HANDLER";
	document.forms[0].ACTIONS.value = "EDIT_PURCHASE_RETURN";
	fnFinalSubmit();
}
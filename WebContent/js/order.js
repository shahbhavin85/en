...
function fnGenerateSalesmanBill(){
	document.forms[0].genBill.disabled = true;
	if(isClick){
		isClick = false;
	} else {
	...
	setTimeout('fnFinalSalesmanGenerateBill()',100);
}

function fnGenerateExhibitionBill(){
	document.forms[0].genBill.disabled = true;
	if(isClick){
		isClick = false;
	} else {
	...
	setTimeout('fnFinalExhibitionGenerateBill()',100);
}
...
function fnFinalSalesmanGenerateBill(){
	if(isClick && document.forms[0].genBill.disabled == true){
		...
		if(document.forms[0].sPriority.value == '--'){
			document.forms[0].genBill.disabled = false;
			isClick = false;
			alert('Please select the priority');
			document.forms[0].sPriority.focus();
			return;
		}
		if(rowsInTable == 1){
			document.forms[0].genBill.disabled = false;
		...
		buildSalesItemData();
		document.forms[0].HANDLERS.value = "SALESMAN_ORDER_HANDLER";
		document.forms[0].ACTIONS.value = "ADD_ORDER";
		fnFinalSubmit();
	}
}

function fnFinalExhibitionGenerateBill(){
	if(isClick && document.forms[0].genBill.disabled == true){
		var vItemTable = document.getElementById('enqItem');
		var rowsInTable = vItemTable.rows.length;
		if(document.forms[0].sAccessPoint.value == '--'){
		...
		buildSalesItemData();
		document.forms[0].HANDLERS.value = "EXHIBITION_ORDER_HANDLER";
		document.forms[0].ACTIONS.value = "ADD_ORDER";
		fnFinalSubmit();
	}
}
...
function printOrder(billNo, copyType){
	window.open('/en/app?HANDLERS=ORDER_HANDLER&ACTIONS=PRNT_ORDER&billNo='+billNo+'&copies='+copyType,"","status=1,scrollbars=1, top=0, left=0,width="+(screen.width-10)+",height="+(screen.height-120));
}

function printReadyNotice(billNo){
	window.open('/en/app?HANDLERS=ORDER_HANDLER&ACTIONS=PRNT_ORDER&billNo='+billNo+'&copies=0&complete=Y',"","status=1,scrollbars=1, top=0, left=0,width="+(screen.width-10)+",height="+(screen.height-120));
}

function emailReadyNotice(billNo){
	if(document.forms[0].email){
		document.forms[0].email.value = "Y";
	}
	document.forms[0].txtComplete.value = "Y";
	document.forms[0].txtInvoiceNo.value = billNo;
	document.forms[0].HANDLERS.value = "ORDER_HANDLER";
	document.forms[0].ACTIONS.value = "EMAIL_ORDER";
	fnFinalSubmit();
}
...
function editSalesBill(billNo){
	document.forms[0].txtInvoiceNo.value = billNo;
	document.forms[0].HANDLERS.value = "ORDER_HANDLER";
	document.forms[0].ACTIONS.value = "INIT_EDIT_ORDER";
	fnFinalSubmit();
}
...
function fnEditGenerateBill(){
	...
	buildSalesItemData();
	document.forms[0].HANDLERS.value = "ORDER_HANDLER";
	document.forms[0].ACTIONS.value = "EDIT_ORDER";
	fnFinalSubmit();
}
...
//	var opt = document.forms[0].sCustomer.options[document.forms[0].sCustomer.selectedIndex];
	...
	/*document.forms[0].txtTin.value = opt.id.substring(opt.id.indexOf("<")+1, opt.id.indexOf(">"));
	checkBlank(document.forms[0].txtTin);
	document.forms[0].txtCst.value = opt.id.substring(opt.id.indexOf(">")+1, opt.id.indexOf("~"));
	checkBlank(document.forms[0].txtCst);
	document.forms[0].txtContactPerson.value = opt.id.substring(opt.id.indexOf("~")+1, opt.id.indexOf("|"));
	checkBlank(document.forms[0].txtContactPerson);
	document.forms[0].txtMobile1.value = opt.id.substring(opt.id.indexOf("|")+1);
...
function fnSelectedType(type){
	if(type == 0){
		document.getElementById("divCredit").style.display = 'none';
		document.getElementById("divEMI").style.display = 'none';
	} else if(type == 1){
		document.getElementById("divCredit").style.display = 'block';
		document.getElementById("divEMI").style.display = 'none';
	} else {
		document.getElementById("divCredit").style.display = 'none';
		document.getElementById("divEMI").style.display = 'block';
	} 
}

function emailOrderBill(billNo){
	if(document.forms[0].email){
		document.forms[0].email.value = "Y";
	}
	document.forms[0].txtInvoiceNo.value = billNo;
...
function smsOrderBill(billNo){
	if(document.forms[0].sms){
